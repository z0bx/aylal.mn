...
export default function PageLogin({ setPage }) {
  ...
  // Формын өгөгдлийг хадгалах төлөвүүд
  ...
  const handleLogin = async () => {
    try {
      setErrorMsg("");
      if (!email || !password) {
        setErrorMsg("Имэйл болон нууц үгээ гүйцэд оруулна уу.");
        return;
      }
  ...
  const handleRegister = async () => {
    try {
      ...
      setErrorMsg(err.message || "Бүртгүүлэхэд алдаа гарлаа.");
    }
  };
    ...
    <main style={{ minHeight: "calc(100vh - 80px)", display: "flex", overflow: "hidden" }}>
      ...
      <div style={{ flex: 2, background: T.surface, display: "flex", alignItems: "center", justifyContent: "center", padding: "80px 64px" }}>
        <div style={{ width: "100%", maxWidth: 380 }} className="fu2">
          ...
          {/* Алдааны мессеж */}
          {errorMsg && (
            <div style={{ color: "#d32f2f", backgroundColor: "#ffebee", padding: "12px 16px", borderRadius: 6, fontSize: 13, marginBottom: 20, fontWeight: 500 }}>
              {errorMsg}
            </div>
          )}
          ...
          {tab === "login" ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em", color: T.onSurfaceVariant }}>Имэйл хаяг</label>
                <input
                  type="email"
                  placeholder="example@nomad.mn"
                  value={email}
                  onChange={e => setEmail(e.target.value)}
                  style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${T.outlineVariant}66`, padding: "12px 0", fontFamily: "'Work Sans',sans-serif", fontSize: 15, color: T.onSurface, outline: "none" }}
                />
              </div>
              ...
              <button
                onClick={handleLogin}
                style={{ padding: "20px 32px", borderRadius: 6, background: T.primary, color: T.onPrimary, border: "none", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
              >
                <span>Аяллаа үргэлжлүүлэх</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em", color: T.onSurfaceVariant }}>Нэр</label>
                <input type="text" placeholder="Таны нэр" value={name} onChange={e => setName(e.target.value)} style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${T.outlineVariant}66`, padding: "12px 0", fontFamily: "'Work Sans',sans-serif", fontSize: 15, color: T.onSurface, outline: "none" }} />
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em", color: T.onSurfaceVariant }}>Имэйл хаяг</label>
                <input type="email" placeholder="example@nomad.mn" value={email} onChange={e => setEmail(e.target.value)} style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${T.outlineVariant}66`, padding: "12px 0", fontFamily: "'Work Sans',sans-serif", fontSize: 15, color: T.onSurface, outline: "none" }} />
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <label style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.15em", color: T.onSurfaceVariant }}>Нууц үг</label>
                <input type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${T.outlineVariant}66`, padding: "12px 0", fontFamily: "'Work Sans',sans-serif", fontSize: 15, color: T.onSurface, outline: "none" }} />
              </div>
              <button
